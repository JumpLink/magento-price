onload = function() {
  ...
  db.magento.init(function (err) {
    splash.show(); // show splash after page is ready
    var async = require('async');
    var fs = require('fs');
    var JADE_PATH = "./app/templates";
    var LESS_PATHS = ["./app/stylesheets", "./app/bower_components", "."];
    var INDEX_PATH = "./app";
    var WATCHED = [];
    ...
    var render_all_jade_files_and_save = function (cb) {
      async.each(fs.readdirSync(JADE_PATH), function (file, cb) {
        check_and_render_jade_file_and_save (JADE_PATH, file, cb );
      }, cb);
    }
